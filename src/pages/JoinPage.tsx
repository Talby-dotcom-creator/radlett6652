...
import { CMSFAQItem, CMSPageContent } from '../types';
...
const JoinPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadPageData = async () => {
      try {
        ...
        // Load both FAQs and page content using optimized API with caching
        const [faqData, contentData] = await Promise.all([
          dataCache.get(CACHE_KEYS.FAQ_ITEMS, () => optimizedApi.getFAQItems(), 20 * 60 * 1000), // 20 min cache
          dataCache.get(CACHE_KEYS.PAGE_CONTENT('join'), () => optimizedApi.getPageContent('join'), 30 * 60 * 1000) // 30 min cache
        ]);
  ...
  const introTitle = pageContent.intro_title || 'Becoming a Freemason';
  const introSubtitle = pageContent.intro_subtitle || 'Freemasonry welcomes men of good character who believe in a Supreme Being and want to contribute to their communities.';
  const introParagraph1 = pageContent.intro_paragraph_1 || 'Joining Radlett Lodge is the beginning of a lifelong journey of personal development, fellowship, and service. Our members come from all walks of life, backgrounds, and beliefs, united by shared values and a desire to make a positive impact on the world.';
  const introParagraph2 = pageContent.intro_paragraph_2 || 'The process of becoming a Freemason is thoughtful and deliberate. We take time to get to know potential members, and for them to get to know us, ensuring that Freemasonry is the right path for each individual.';
  const requirementsIntro = pageContent.requirements_intro || 'To be eligible for membership in Radlett Lodge No. 6652, you must meet the following criteria:';
  const financialIntro = pageContent.financial_intro || 'Membership in Radlett Lodge, as with all Masonic Lodges, involves certain financial obligations. These typically include:';
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <div 
                ...
                dangerouslySetInnerHTML={{ __html: introParagraph1 }}
              />
              <div 
                className="text-neutral-600"
                dangerouslySetInnerHTML={{ __html: introParagraph2 }}
              />
            </div>
            <img
              src="https://images.pexels.com/photos/8837750/pexels-photo-8837750.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
              alt="Masonic Symbols"
        ...
        <div className="container mx-auto px-4 md:px-6">
          ...
          <div className="max-w-3xl mx-auto mt-12">
            <div className="bg-white rounded-lg shadow-medium p-6 md:p-8">
              <div 
                className="mb-6 text-neutral-600"
                dangerouslySetInnerHTML={{ __html: financialIntro }}
              />
