...
const ContactPage: React.FC = () => {
  ...
  useEffect(() => {
    const loadContactData = async () => {
      try {
        // Load both settings and page content using optimized API with caching
        const [siteSettings, contentData] = await Promise.all([
          dataCache.get(CACHE_KEYS.SITE_SETTINGS, () => optimizedApi.getSiteSettings(), 30 * 60 * 1000), // 30 min cache
          dataCache.get(CACHE_KEYS.PAGE_CONTENT('contact'), () => optimizedApi.getPageContent('contact'), 30 * 60 * 1000) // 30 min cache
        ]);
  ...
  const introText = pageContent.intro_text || 'We welcome your inquiries about Radlett Lodge and Freemasonry.';
  const visitingInfo = pageContent.visiting_info || 'If you\'re a Freemason planning to visit Radlett Lodge, please contact our Secretary beforehand. Proof of membership in a recognized Masonic Lodge will be required.';
            ...
            <div>
              ...
              <div className="bg-neutral-50 p-6 rounded-lg shadow-soft">
                ...
                <div 
                  ...
                  dangerouslySetInnerHTML={{ __html: visitingInfo }}
                />
                <p className="text-neutral-600">
