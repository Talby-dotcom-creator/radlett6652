  ...
  useEffect(() => {
    const loadPageContent = async () => {
      try {
        ...
        // Get all page content for the terms page using optimized API with caching
        const content = await dataCache.get(
          CACHE_KEYS.PAGE_CONTENT('terms'), 
          () => optimizedApi.getPageContent('terms'), 
          60 * 60 * 1000 // 1 hour cache for terms content (rarely changes)
        );
